@echo off
REM ============================================================================
REM eMark Launcher Script for Windows
...
if not "%JAVA_MAJOR%"=="8" (
    echo WARNING: Java %JAVA_VERSION% detected. eMark requires Java 8 ^(1.8.x^).
    echo.
...
set JAR_FILE=
for %%f in (target\eMark-*.jar) do set JAR_FILE=%%f

if not defined JAR_FILE (
    REM Try current directory
    for %%f in (eMark-*.jar) do set JAR_FILE=%%f
)
