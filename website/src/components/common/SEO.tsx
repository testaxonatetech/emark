...
export function SEO({
  ...
  const fullTitle = title ? `${title} | eMark` : DEFAULT_TITLE;
...
export const structuredDataGenerators = {
  softwareApplication: () => ({
    ...
    description: 'eMark is a free, open-source PDF signing software and modern alternative to jSignPDF and Adobe Reader DC. Sign PDF documents with USB DSC tokens, PKCS#11 hardware tokens, and PKCS#12/PFX certificates. Creates legally valid digital signatures with green tick checkmark verification.',
    url: BASE_URL,
    downloadUrl: 'https://github.com/cispl-dev/emark/releases/latest/download/eMark-Setup.exe',
    softwareVersion: '1.0',
    ...
    author: {
      '@type': 'Organization',
      name: 'TrexoLab',
      url: 'https://github.com/cispl-dev',
    },
    publisher: {
      '@type': 'Organization',
      name: 'TrexoLab',
  ...
  howTo: (title: string, description: string, steps: { name: string; text: string; image?: string }[]) => ({
    ...
    tool: {
      '@type': 'HowToTool',
      name: 'eMark PDF Signing Software',
    },
    supply: [
      {
        '@type': 'HowToSupply',
  ...
  imageGallery: (images: { url: string; title: string; description: string }[]) => ({
    '@context': 'https://schema.org',
    '@type': 'ImageGallery',
    name: 'eMark PDF Signing Tool Screenshots',
    description: 'Gallery of eMark application screenshots showing PDF signing features',
